...
$hookScript = Join-Path $targetDir 'review-pre-commit.ps1'
$wcPath = (Resolve-Path $WorkingCopyPath).Path

Write-Host ""
Write-Host "=== TortoiseSVN Pre-commit Hook 配置 ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$hookScript`" %PATH% %DEPTH% %MESSAGEFILE% %CWD%" -ForegroundColor White
Write-Host ""
